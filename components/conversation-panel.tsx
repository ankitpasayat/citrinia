"use client";

// The list pane: the heading, the way to start a new one, the Inbox/Requests
// control and the conversations themselves.
//
// A client component for one reason: the tab lives in `?tab=requests`, and a
// layout -- which is where this is rendered, so the list survives moving between
// conversations -- is not given searchParams on the server. The rows arrive as
// props from that layout, so nothing is fetched here; only the reading of one
// query parameter is client-side.
import * as stylex from "@stylexjs/stylex";
import Link from "next/link";
import { useSearchParams } from "next/navigation";
import { colors, fonts, shape } from "@/app/tokens.stylex";
import type { Thread } from "@/lib/conversations";
import { isRequest } from "@/lib/messages";
import { buttonStyles } from "./button";
import { ConversationList } from "./conversation-list";
import { EmptyState } from "./empty-state";
import { PlusIcon } from "./icons";
import { RequestActions } from "./request-actions";
import { Segmented } from "./segmented";

export function ConversationPanel({ threads, viewerId }: { threads: Thread[]; viewerId: string }) {
  // Anything that is not a tab name is the inbox, so a hand-edited url renders.
  const onRequests = useSearchParams().get("tab") === "requests";

  // One read, split by the same rule the badge counts by. A request is only ever
  // a request to the person who did not start it.
  const requests = threads.filter((t) => isRequest(t.conversation, viewerId));
  const inbox = threads.filter((t) => !isRequest(t.conversation, viewerId));
  const shown = onRequests ? requests : inbox;

  return (
    <>
      <div {...stylex.props(styles.head)}>
        <h1 {...stylex.props(styles.heading)}>Messages</h1>
        <Link
          href="/messages/new"
          aria-label="New message"
          {...stylex.props(buttonStyles.base, buttonStyles.variants.icon)}
        >
          <PlusIcon />
        </Link>
      </div>

      {/* The tab only appears once there is something behind it: an empty
          Requests tab is a door onto a cupboard. */}
      {requests.length > 0 && (
        <Segmented
          label="Messages"
          segments={[
            { href: "/messages", label: "Inbox", current: !onRequests },
            {
              href: "/messages?tab=requests",
              // A dot, not a number: what matters is that somebody is waiting,
              // and a count here would be a second badge in the app.
              label: (
                <>
                  Requests
                  <span
                    role="img"
                    aria-label="Waiting"
                    {...stylex.props(styles.dot, onRequests && styles.dotOn)}
                  />
                </>
              ),
              current: onRequests,
            },
          ]}
        />
      )}

      {shown.length === 0 ? (
        onRequests ? (
          <EmptyState title="No requests" body="Messages from people you don't follow wait here." />
        ) : (
          <EmptyState
            title="No messages yet"
            body="Say hello to somebody. Their profile has a Message button, or tap + to find them."
          />
        )
      ) : (
        <ConversationList
          threads={shown}
          viewerId={viewerId}
          action={
            onRequests
              ? ({ conversation, other }) => (
                  <RequestActions
                    conversationId={conversation.id}
                    profileId={other.id}
                    handle={other.username}
                  />
                )
              : undefined
          }
        />
      )}
    </>
  );
}

const styles = stylex.create({
  head: { display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12 },
  heading: {
    margin: 0,
    fontFamily: fonts.display,
    fontWeight: 400,
    fontSize: "1.625rem",
    lineHeight: 1.15,
    color: colors.burnt,
  },
  dot: {
    display: "inline-block",
    width: 7,
    height: 7,
    marginInlineStart: 6,
    borderRadius: shape.pill,
    backgroundColor: colors.burnt,
  },
  // On the selected segment the pill is already burnt, so the dot takes the
  // segment's own colour to stay visible on it.
  dotOn: { backgroundColor: colors.onButton },
});
